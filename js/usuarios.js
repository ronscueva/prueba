...
function registrarcli() {
    ...
    if (nombre == "") {
        ...
        Toast.fire({ icon: 'error', title: 'Debe de Ingresar el DNI.' })
        return;
    } else if (apellido == "") {
        Toast.fire({ icon: 'error', title: 'Debe de Ingresar el Apellido.' })
        return;
    }
    $.ajax({
        type: "post",
        url: "crud_usuarios.php",
        data: "funcion=" + 1 + "&id=" + 0 + "&nombre=" + nombre +  "&apellido=" + apellido + "&dni=" + dni + "&usuario=" + usuario + "&pass=" + pass + "&level=" + level,
        success: function (data) {
            console.log(data);
            if (data == 1) {
                var Toast = Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
