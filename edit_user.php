...
?>
<?php
  $e_user = find_by_id('empp_tb_users',(int)$_GET['id']);
  ...
  if(isset($_POST['update'])) {
    ...
    if(empty($errors)){
       ...
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE empp_tb_users SET name ='{$name}', username ='{$username}',user_level='{$level}',status='{$status}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
          $sql = "UPDATE empp_tb_users SET password='{$h_pass}' WHERE id='{$db->escape($id)}'";
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
